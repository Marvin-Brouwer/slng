...
export class ResponsePanel implements vscode.WebviewViewProvider {
	...
	resolveWebviewView(view: vscode.WebviewView) {
		...
		this.context.addSubscriptions(
			view.webview.onDidReceiveMessage((message: { command: string, content?: string, reference?: number, source?: 'response' | 'request' }) => {
	...
	public async show(reference: string | undefined) {
		...
		if (!this.context.responseCache.has(reference)) {
			this.view.webview.html = this.noSelectionView()
			return
		}

		const referencedResponse = this.context.responseCache.get(reference)
		...
		const view = `
			...
			<vscode-panels aria-label="Request information">
				...
				<vscode-panel-view id="view-response">
					<http-response>${JSON.stringify(response.responseAst, undefined, __DEV__ ? 2 : 0)}</http-response>
				</vscode-panel-view>
				<vscode-panel-view id="view-request">
					<http-request>${JSON.stringify(response.request.templateAst, undefined, __DEV__ ? 2 : 0)}</http-request>
				</vscode-panel-view>
			</vscode-panels>
		`
